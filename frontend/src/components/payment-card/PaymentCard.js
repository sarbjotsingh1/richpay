...
export default function PaymentCard() {
  ...
  const { user, showAlert, addSubscriptionDetialsToDatabase, baseUrl } =
        ...
        if (status == "requires_action") {
          stripe.confirmCardPayment(client_secret).then((result) => {
            ...
            if (result.error)
              ...
              addSubscriptionDetialsToDatabase(
                state,
    ...
    <div className="payment__card">
      ...
      <div className="order__info">
        ...
        <ul>
          <li>
            Plan Name
            <span className="incapital">{state && state.plan}</span>
          </li>
          <hr />
          <li>
            Billing Cycle
            <span className="incapital">{state && state.period}</span>
          </li>
          <hr />
          <li>
            Plan Price
            <span>
              &#8377;{" "}
              {state && state.period == "month"
                ? state.planInfo["Monthly-Price"] + "/mo"
                : state.planInfo["Yearly-Price"] + "/yr"}
            </span>
          </li>
          <hr></hr>
        </ul>
