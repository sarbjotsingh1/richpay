    ...
    <div className="card plan__card">
      <div className="card__header">
        <div className="header__left">
          ...
          {type === "active" ? (
            <span className="active__badge">Active</span>
          ) : (
      ...
      {plan === "mobile" ? (
        <p className="device-select">Phone+Tablet</p>
      ) : (
        <p>Phone+Tablet+Computer+TV</p>
      )}

      <h1>
        &#8377; {amount}/<sub style={{ fontWeight: "normal" }}>{period}</sub>
      </h1>
      ...
      >
        {type === "active" ? "Change Plan" : "Choose Plan"}
      </button>

      {type === "active" ? (
        <p>
          Your subscription has started from now and will auto renew itself.
        </p>
      ) : (
        <p>
